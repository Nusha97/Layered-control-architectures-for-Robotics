function [K,P] = finite_horizon_dlqr_quad(A, B, v)
...
% %% load inverted pendulum linearized dynamics
% tau = .1;
% sys_ss = inv_pendulum();
% dsys_ss = c2d(sys_ss,tau);

% %sinusoidal reference trajectory
% v1 = [1*sin(2*pi*t/50)];
% v2 = [1*2*pi/50*cos(2*pi*t/50)];
% v = [v1;v2];

%trackign penalty weight
rho = 1000;
...
% % lA = [1,1*tau;0,1];
% % A = blkdiag(lA,lA);
% % A = [lA,.1*randn(2,2);.1*randn(2,2),lA];
% % lB = [0;tau];
% % B = blkdiag(lB,lB);
% A = dsys_ss.A;
% B = dsys_ss.B;
