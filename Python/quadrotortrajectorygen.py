...
class SingleAxisTrajectory:
    ...
    def generate(self, Tf):
        ...
        # Calculate the cost:
        self._cost = (self._g ** 2) + self._b * self._g * Tf + (
                    self._b ** 2) * T2 / 3.0 + self._a * self._g * T2 / 3.0 + self._a * self._b * T3 / 4.0 + (
                                 self._a ** 2) * T4 / 20.0
    ...
    def get_velocity(self, t):
        ...
        return self._v0 + self._a0 * t + (1.0 / 2.0) * self._g * t * t + (1.0 / 6.0) * self._b * t * t * t + (
                    1.0 / 24.0) * self._a * t * t * t * t

    def get_position(self, t):
        """Return the scalar position at time t."""
        return self._p0 + self._v0 * t + (1.0 / 2.0) * self._a0 * t * t + (1.0 / 6.0) * self._g * t * t * t + (
                    1.0 / 24.0) * self._b * t * t * t * t + (1.0 / 120.0) * self._a * t * t * t * t * t
    ...
    def _check_input_feasibility_section(self, fminAllowed, fmaxAllowed,
        ...
        if (fmin < fminAllowed) or (fmax > fmaxAllowed) or (wBound > wmaxAllowed):
            ...
            if r1 == InputFeasibilityResult.Feasible:
                ...
                return self._check_input_feasibility_section(fminAllowed, fmaxAllowed, wmaxAllowed, minTimeSection,
                                                             tHalf, t2)
            else:
